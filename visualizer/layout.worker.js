...
/**
 * Optional Dagre-based layout. Dynamically loads dagre inside the worker and
 * falls back to the default Kahn layout if dagre isn't available or the
 * import fails.
 *
 * This function expects `mappings` in the same shape as the regular layout.
 */
const getDagreLayout = (mappings) => {
    try {
        // Try to import dagre inside the worker. importScripts is synchronous
        // and works in workers when allowed by the hosting environment.
        importScripts('https://unpkg.com/dagre@0.8.5/dist/dagre.min.js');
    } catch (e) {
        // If import fails, return null and the caller will fall back.
        return null;
    }

    if (typeof self.dagre === 'undefined' && typeof dagre === 'undefined') {
        return null;
    }

    const dagreLib = (typeof self.dagre !== 'undefined') ? self.dagre : dagre;
    ...
    for (const m of mappings) {
        ...
        for (const cs of m.callSites) {
            ...
            if (!edges.has(edgeId)) edges.add(edgeId);
        }
    }
    ...
    // Add nodes with heuristic sizes
    for (const n of nodes.values()) {
        // approximate node width/height to help dagre avoid overlaps
        g.setNode(n.id, { width: 200, height: 80, data: n });
    }

    // Add edges
    for (const edgeId of edges) {
        const [source, target] = edgeId.split('->');
        g.setEdge(source, target, { id: edgeId });
    }
    ...
    try {
        ...
        // If dagre fails for any reason, fallback
        return null;
    }

    // Map back to nodes/edges arrays used by ReactFlow
    const initialNodes = g.nodes().map(id => {
        const nd = g.node(id);
    ...
    const initialEdges = g.edges().map(e => {
        const edgeMeta = g.edge(e);
        return { id: edgeMeta.id || `${e.v}->${e.w}`, source: e.v, target: e.w };
    });
